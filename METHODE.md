...
## Überstunden aus der Mitarbeiter-App (seit 23.09.2026, Valentin)

Quelle: Supabase Edge Function `get-overtime-for-reports` (Anträge mit Status offen oder genehmigt, getrennt in Therapiezeit und Freizeitausgleich, je MediFox-ID und Tag; `SUPABASE_ANON_KEY` aus `~/.claude/.env` bzw. GitHub-Secret, für Tests `OVERTIME_JSON=<datei>`). Regel im Nenner von `compute_quartal`, identisch zum PM-Wochenreport: (1) alle erbrachten Termine zählen; (2) der Nenner wächst je Tag eins zu eins um min(beantragte Therapie-Minuten, erbrachte Behandlung außerhalb der verfügbaren Zeit), verfügbare Zeit = Arbeitszeit-Slots des Tages, an Abwesenheits- (alle Arten) und Feiertagen null, Termin halb außerhalb anteilig, ohne Parallel-Positionen; (3) Freizeitausgleich senkt die Vertragszeit an Werktagen um min(App-Minuten, Tagesstunden), null an MediFox-Abwesenheitstagen. Stichtag `OT_AB` = 01.07.2026. Ergebnisfelder `ot_ber`, `ausgl_ber`, `ot_ausser_h`; Leitungszeit der Standortleitung auf die Zeit ohne Ausgleich bezogen. Fehlen die Daten, wirkt die Regel nicht (Warnung im Log).

## Nicht gewertete Therapeuten (seit 25.09.2026, Valentin)

Konstante `NICHT_GEWERTET` in `generate.py` (Stufe 1; dieselbe Tabelle im PM- und Management-Wochenreport, im SL-Node und im App-Auslastungs-Workflow, immer zusammen pflegen). Ab dem Datum `ab` endet das Bewertungsfenster des Therapeuten am Tag davor: weder Umsatz noch Nenner zählen ab dann (symmetrisch zur 29-Tage-Regel am Anfang). Ohne Datum keine Wirkung. Konstante heute nur Theda Fallois ab 01.07.2026 (selbst-managend). Gekündigte kommen automatisch aus der Mitarbeiter-App: `_overtime_data` liest `ausschluss` der Edge Function (Mitarbeiter mit Austrittsdatum, `ab` = Tag, an dem HR das Austrittsdatum eingetragen hat, Spalte `exit_date_set_at` per Trigger, Migration 20260925120000). Ein MediFox-Beschäftigungsende ist kein Signal (Kontowechsel zwischen Standorten).
